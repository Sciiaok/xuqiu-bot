...
async function callModel(model, { system, userPrompt, maxTokens, tenantId, callSite }) {
 ...
 */
export async function generateSummaryWithFallback({ system, userPrompt, maxTokens = 500, logTag = 'ai-summary', tenantId = null, callSite = 'ai-summary' }) {
  try {
    const text = await callModel(MODELS.MINIMAX, { system, userPrompt, maxTokens, tenantId, callSite });
    if (text) return text;
  ...
  const text = await callModel(MODELS.HAIKU, { system, userPrompt, maxTokens, tenantId, callSite });
