...
export async function teach(agentId, content, layer) {
  return apiFetch('/api/knowledge/teach', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ agent_id: agentId, content, layer }),
  });
}
...
export async function listSessions(agentId) {
  const data = await apiFetch(`/api/knowledge/test-chat/sessions${qs({ agent_id: agentId })}`);
  return data.sessions || [];
}

export async function getSession(sessionId) {
  const data = await apiFetch(`/api/knowledge/test-chat/sessions/${sessionId}`);
  return data.messages || [];
}

export async function sendMessage(agentId, sessionId, message) {
  return apiFetch('/api/knowledge/test-chat', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteSession(sessionId) {
  return apiFetch(`/api/knowledge/test-chat/sessions/${sessionId}`, { method: 'DELETE' });
}
