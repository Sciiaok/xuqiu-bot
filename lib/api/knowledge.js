...
export async function getHealth(productLineId) {
  return apiFetch(`/api/knowledge/health${qs({ product_line_id: productLineId })}`);
}

/* ── Documents ────────────────────────────────────────── */

export async function listDocuments(productLineId) {
  const data = await apiFetch(`/api/knowledge/documents${qs({ product_line_id: productLineId })}`);
 ...
 */
export async function uploadDocument(productLineId, file, layer) {
 ...
 */
export async function teachExtract(productLineId, message) {
  return apiFetch('/api/knowledge/teach', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ product_line_id: productLineId, message }),
  });
}
 ...
 */
export async function teachCommit(productLineId, items) {
  return apiFetch('/api/knowledge/teach/commit', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ product_line_id: productLineId, items }),
  });
}

/* ── Assets (sendable images Medici can attach to a reply) ── */

export async function listAssets(productLineId) {
  const data = await apiFetch(`/api/knowledge/assets${qs({ product_line_id: productLineId })}`);
  return data.assets || [];
}

export async function uploadAsset(productLineId, file, description, tags = {}) {
  const formData = new FormData();
  formData.append('file', file);
  formData.append('product_line_id', productLineId);
...
export async function listQaSnippets(productLineId, { includeInactive = false } = {}) {
  const data = await apiFetch(`/api/knowledge/qa-snippets${qs({ product_line_id: productLineId, include_inactive: includeInactive ? 'true' : '' })}`);
  return data.snippets || [];
}

export async function updateQaSnippet(productLineId, snippetId, patch) {
  return apiFetch('/api/knowledge/qa-snippets', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ product_line_id: productLineId, snippet_id: snippetId, ...patch }),
  });
}

export async function deleteQaSnippet(productLineId, snippetId) {
  return apiFetch(`/api/knowledge/qa-snippets${qs({ product_line_id: productLineId, snippet_id: snippetId })}`, { method: 'DELETE' });
}

/* ── Pending review ──────────────────────────────────── */

export async function listPendingReview(productLineId, { status = 'pending' } = {}) {
  const data = await apiFetch(`/api/knowledge/pending-review${qs({ product_line_id: productLineId, status })}`);
  return data.items || [];
}

export async function resolvePendingReview(productLineId, reviewId, action, { note } = {}) {
  return apiFetch('/api/knowledge/pending-review', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ product_line_id: productLineId, review_id: reviewId, action, note }),
  });
}

/* ── Corrections ─────────────────────────────────────── */

export async function listCorrections(productLineId, { status = 'pending' } = {}) {
  const data = await apiFetch(`/api/knowledge/corrections${qs({ product_line_id: productLineId, status })}`);
  return data.items || [];
}

export async function recordCorrection(productLineId, payload) {
  return apiFetch('/api/knowledge/corrections', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ product_line_id: productLineId, ...payload }),
  });
}

export async function resolveCorrection(productLineId, correctionId, action, { overrides } = {}) {
  return apiFetch('/api/knowledge/corrections', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ product_line_id: productLineId, correction_id: correctionId, action, overrides }),
  });
}
