...
export function buildInquiryRecords({ conversations, leads, productLineMap }) {
  ...
  return conversations.map(conversation => {
    ...
    const productLine = conversation.product_line || null;
    const line = productLine ? productLineMap[productLine] : null;
    const primaryIntent = primaryLead ? parseIntents(primaryLead.conversation_intent)[0] : null;

    return {
      conversationId: conversation.id,
      productLine: productLine || 'unknown',
      productLineName: line?.name || 'Unknown',
      ...
      productName: primaryLead ? getProductName(primaryLead, productLine || 'unknown') : null,
    };
  });
}
...
async function fetchProductLineMap(supabase, tenantId, productLines) {
  // 显示用 lookup（name 给 inquiry row 渲染）。无论 is_active，全量取——
  // 历史 conversation 落在已禁用的产品线上也得能渲染出 name。
  let query = supabase
    .from('product_lines')
    .select('id, name')
    .eq('tenant_id', tenantId);
  if (productLines && productLines.length > 0) {
    query = query.in('id', productLines);
  }
  ...
  const lines = data || [];
  return {
    productLineIds: lines.map((l) => l.id),
    productLineMap: Object.fromEntries(lines.map((l) => [l.id, l])),
  };
}
...
async function queryConversations(supabase, tenantId, fromISO, toISO) {
  return fetchAllPages(() => (
    supabase
      .from('conversations')
      .select('id, product_line, created_at, last_message_at')
      .eq('tenant_id', tenantId)
...
async function queryLeads(supabase, tenantId, conversationIds) {
  ...
  for (let i = 0; i < conversationIds.length; i += batchSize) {
    ...
    const rows = await fetchAllPages(() => (
      supabase
        .from('leads')
        .select('id, inquiry_quality, business_value, conversation_intent, details, conversation_id, created_at')
        .eq('tenant_id', tenantId)
...
function buildProductLineDistribution(inquiries) {
  const groups = {};
  for (const inquiry of inquiries) {
    const key = inquiry.productLine || '__unknown__';
    const g = groups[key] || (groups[key] = {
      productLine: inquiry.productLine || null,
      name: inquiry.productLineName,
      inquiryCount: 0,
      proofCount: 0,
...
function buildTopProducts(inquiries) {
  ...
  for (const inquiry of inquiries) {
    ...
    const g = groups[key] || (groups[key] = {
      ...
      productLineName: inquiry.productLineName,
      inquiryCount: 0,
      proofCount: 0,
    });
    g.inquiryCount += 1;
  ...
  return Object.values(groups)
    .map((g) => ({
      ...
      productLineName: g.productLineName,
...
export async function fetchDashboardData(supabase, { tenantId, windows, productLines }) {
  ...
  // productLineMap 给 inquiry row 渲染 name；不参与过滤——窗口内全部 conversation
  // 都进。
  const { productLineMap } = await fetchProductLineMap(supabase, tenantId, productLines);
  ...
  const current = buildInquiryRecords({ conversations: currentConvs, leads: currentLeads, productLineMap });
  const previous = buildInquiryRecords({ conversations: previousConvs, leads: previousLeads, productLineMap });
  ...
  return {
    ...
    productLineDistribution: buildProductLineDistribution(current),
...
export function parseDashboardParams(searchParams) {
  ...
  // Empty default = "全部产品线" — fetchProductLineMap() 不传 productLines 时
  // 默认拉全租户，仪表盘自动包含新建产品线。
