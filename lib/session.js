...
/**
 * 一条 Claude 吐出的 lead 是否「成立」。业务字段都在 details JSONB（产品线
 * lead_fields 可配置，不能按 car_model / product_name 硬编码判断）；details 里
 * 有任意非空字段即算有效，generic 回退形态的顶层字段一并兜底。
 * 唯一真源 —— processMessageWithSession 与 queue-processor 的强制转人工门槛共用。
 */
export function isValidLead(lead) {
  return Boolean(
    (lead?.details && Object.keys(lead.details).length > 0) ||
    lead?.product_name || lead?.car_model,
  );
}

export function hasValidLeads(leads) {
  return Array.isArray(leads) && leads.some(isValidLead);
}
...
async function processMessageWithSession(session, userMessageContent, claudeResponse) {
  ...
  //    留住返回行的 id —— 上游 queue-processor 在 sendMessage 拿到 wamid 后,
  //    要按这个 id 把 delivery/wamid 回填进 metadata(见 markBotMessageSent)。
  ...
  const updatedSession = await getSessionByConversationId(session.conversation_id);
  // 透出本轮 bot 回复的 message id(可能为 null,空回复时不建消息),供
  // queue-processor 发送成功后回填投递状态。
  updatedSession._assistantMessageId = assistantMessage?.id || null;
