...
async function processMessageWithSession(session, userMessageContent, claudeResponse) {
  ...
  // 业务字段（含 car_model / product_name）现在都在 lead.details 里 ── normalizer
  // 已经把顶层硬编码字段全部移进 details JSONB。
  const validLeads = leadsData.filter(lead => lead.details?.car_model || lead.details?.product_name);
