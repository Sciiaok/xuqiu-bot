 ...
 * phoneNumberId is preserved as context only; agent selection happens later via router.
 * @param {Object} params
 * @param {string} params.waId - WhatsApp user ID
 * @param {string} [params.profileName] - Contact name from webhook profile
 * @param {string} [params.phoneNumberId] - WhatsApp phone number ID (for context/logging)
  ...
  if (latestActiveConversation && !isConversationStale(latestActiveConversation.last_message_at)) {
    conversation = latestActiveConversation.wa_phone_number_id !== (phoneNumberId || null)
      ? await getOrCreateConversation(
        contact.id,
        latestActiveConversation.agent_id || null,
        phoneNumberId || null
      )
