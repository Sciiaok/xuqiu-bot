...
export async function processConversationQueue(conversationId) {
  ...
  try {
    ...
    // 7.5 营业时段长对话强制转人工 —— 叠加在 Medici 路由判断之上的硬规则。
    //   北京时间 10–20 点、对话已超过三轮、且模型本轮判 CONTINUE、且本轮抽到
    //   有效线索时，把 route 改写为 HUMAN_NOW。必须在落库前改 —— leads 以
    //   HUMAN_NOW 落库后，下游 executeConversationRouting 才查得到并发飞书 +
    //   起接管（它按 leads.route 列过滤）。仅覆盖 CONTINUE：模型主动判的
    //   FAQ_END / HUMAN_NOW 一律尊重，不强转。无有效线索时不触发（纯闲聊 /
    //   FAQ 不打扰销售）。priorRounds 用落库前历史长度：floor(history/2) >= 3
    //   即本轮为第 4 个客户消息起，落库后对话「超过三轮」。
    const priorRounds = Math.floor((session.messages?.length || 0) / 2);
    const forcedBusinessHoursHandoff = shouldForceBusinessHoursHandoff({
      route: claudeResponse.route,
      priorRounds,
      hasLead: hasValidLeads(claudeResponse.leads),
    });
    if (forcedBusinessHoursHandoff) {
      ...
      if (!claudeResponse.handoff_summary || !claudeResponse.handoff_summary.trim()) {
        claudeResponse.handoff_summary =
          '系统规则：营业时段（北京时间 10–20 点）对话已超过三轮，自动转人工跟进。';
      }
      logger.info('queue.route.forced_human_now_business_hours', {
    ...
    //     被营业时段规则强转的 HUMAN_NOW 不受此降级影响（否则刚强转又被打回
    //     FAQ_END，且 leads 已以 HUMAN_NOW 落库会错配）。模型自判的 HUMAN_NOW
    //     维持既有行为，不在豁免之列。
    let finalRoute = claudeResponse.route;
    if (!forcedBusinessHoursHandoff && hasReachedGlobalMaxTurns(updatedSession.messages?.length || 0)) {
