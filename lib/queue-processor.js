...
import { getSessionByConversationId, processMessageForConversation, hasValidLeads } from './session.js';
...
import { createMessage, markBotMessageSent } from './repositories/message.repository.js';
import {
  ...
  try {
    ...
    const contextInfo = {
      ...
      prior_state: session._lead ? {
        ...
        // 字段名按产品线可配置 —— 走别名感知解析，别只认 car_model/qty_bucket，
        // 否则非 vehicle 线 prior_state 全空、AI 会重复追问已知信息。
    ...
    // 7. 二次闸门：Medici 推理 5-15s，期间操作员可能刚点了「接管对话」。
    //    入口的 checkAndExpireTakeover 这时已经过期，不能阻止 race。这里
    //    在落库 + 发送之前再读一次 takeover 状态——若 true 则整体丢弃
    //    Medici 输出，走和 takeover 分支等价的"只落 customer 消息"通道。
    //    用 pure-read 的 isHumanTakeover，不再触发 expiry。
    if (await isHumanTakeover(conversationId)) {
      logger.warn('queue.takeover.late_detected', {
        intent: claudeResponse.conversation_intent,
        route: claudeResponse.route,
        dropped_reply_preview: (claudeResponse.next_message || '').slice(0, 120),
      });
      const result = await persistMessagesOnly({
        messages, messageIds, conversationId, tenantId, logger, reasonTag: 'takeover_late',
      });
      return { ...result, humanTakeover: true, lateTakeover: true };
    }

    // 7.5 "上班"状态下长对话强制转人工 —— 叠加在 Medici 路由判断之上的硬规则。
    //   产品线处于"上班"(reception_on=true)、对话已超过三轮、且模型本轮判
    //   CONTINUE、且本轮抽到有效线索时，把 route 改写为 HUMAN_NOW。"下班"时整条
    //   规则停用。必须在落库前改 —— leads 以 HUMAN_NOW 落库后，下游
    //   executeConversationRouting 才查得到并发飞书 + 起接管（它按 leads.route 列
    //   过滤）。仅覆盖 CONTINUE：模型主动判的 FAQ_END / HUMAN_NOW 一律尊重，不强转。
    //   无有效线索时不触发（纯闲聊 / FAQ 不打扰销售）。priorRounds 用落库前历史
    //   长度：floor(history/2) >= 3 即本轮为第 4 个客户消息起，落库后「超过三轮」。
    const priorRounds = Math.floor((session.messages?.length || 0) / 2);
    const modelRoute = claudeResponse.route;
    const onDuty = agentConfig.reception_on !== false;
    const hasLead = hasValidLeads(claudeResponse.leads);
    // 落库前先决定"上班强转":命中则改写 route,使 leads 以 HUMAN_NOW 落库(下游
    // executeConversationRouting 按 leads.route 列查),并补 handoff_summary。最终
    // route 在第 10 步由 resolveHostRoute 统一裁定(与调试台同源)。
    if (shouldForceOnDutyHandoff({ route: modelRoute, priorRounds, hasLead, onDuty })) {
      claudeResponse.route = 'HUMAN_NOW';
      if (!claudeResponse.handoff_summary || !claudeResponse.handoff_summary.trim()) {
        claudeResponse.handoff_summary =
          '系统规则：接待处于上班状态，对话已超过三轮，自动转人工跟进。';
      }
      logger.info('queue.route.forced_human_now_on_duty', { prior_rounds: priorRounds });
    }
    ...
    if (claudeResponse.next_message && claudeResponse.next_message.trim() !== '') {
      const sendResult = await sendMessage(waId, claudeResponse.next_message, sessionPhoneNumberId);
      // 回填投递初始态:存 Meta 返回的 wamid + delivery.status='sent',让这条 bot
      // 回复也有 ✓ 角标,并能被后续 statuses webhook 按 wamid 升级成 ✓✓ / 已读 /
      // 失败(与人工发送路径对齐)。best-effort:markBotMessageSent 内部吞错不抛。
      const outboundWamid = sendResult?.messages?.[0]?.id || null;
      await markBotMessageSent(updatedSession._assistantMessageId, outboundWamid);
    ...
    if (Array.isArray(claudeResponse.attachments) && claudeResponse.attachments.length > 0) {
      ...
      const productContext = freshLead ? {
        ...
        // 别名感知 —— 非 vehicle 线靠 model/machinery_type 才能让 attachment-guard
        // 的 SKU token 匹配生效，否则只剩 brand、过滤偏弱。
    ...
    // 10. 宿主最终 route —— 与 medici 调试台共用 resolveHostRoute(单一真源,防两
    //     环境漂移)。传【原始】modelRoute:上班强转→HUMAN_NOW(优先);否则满 30 轮
    //     →FAQ_END;否则原样。被上班规则强转的 HUMAN_NOW 不会被 30 轮降级(优先级
    //     在 resolveHostRoute 内)。
    const host = resolveHostRoute({
      modelRoute,
      priorRounds,
      postMessageCount: updatedSession.messages?.length || 0,
      hasLead,
      onDuty,
    });
    const finalRoute = host.route;
    if (host.reason === 'global_max_turns') {
      logger.warn('queue.route.forced_faq_end', { global_max_turns: getGlobalMaxTurns() });
    }
