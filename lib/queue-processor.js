...
export async function processConversationQueue(conversationId) {
  ...
  try {
    ...
    const forcedHandoff = shouldForceOnDutyHandoff({
      route: claudeResponse.route,
      priorRounds,
    ...
    if (forcedHandoff) {
      ...
      logger.info('queue.route.forced_human_now_on_duty', {
    ...
    //     被"上班"规则强转的 HUMAN_NOW 不受此降级影响（否则刚强转又被打回
    //     FAQ_END，且 leads 已以 HUMAN_NOW 落库会错配）。模型自判的 HUMAN_NOW
    //     维持既有行为，不在豁免之列。
    let finalRoute = claudeResponse.route;
    if (!forcedHandoff && hasReachedGlobalMaxTurns(updatedSession.messages?.length || 0)) {
