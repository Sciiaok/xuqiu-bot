...
export async function processConversationQueue(conversationId) {
  ...
  try {
    ...
    // 7. Guard: block FAQ_END flip when conversation already has business context
    //    Must run BEFORE processMessage and sendMessage to prevent:
    //    - Lead data being overwritten with personal_consumer intent
    //    - Rejection message being sent that poisons future turns
    if (claudeResponse.route === 'FAQ_END' && session._lead) {
      const priorIntent = session._lead.conversation_intent || '';
      const priorQuality = session._lead.inquiry_quality || '';
      const hadBusinessContext =
        /business_inquiry|business_cooperation/.test(priorIntent) ||
        ['GOOD', 'QUALIFY', 'PROOF'].includes(priorQuality);

      if (hadBusinessContext) {
        logger.warn('queue.route.faq_end_blocked', {
          reason: 'prior_business_context',
          prior_intent: priorIntent,
          prior_quality: priorQuality,
          claude_intent: claudeResponse.conversation_intent,
          blocked_message: claudeResponse.next_message?.slice(0, 240),
        });
        claudeResponse.route = 'CONTINUE';
        // Preserve prior business classification so lead data isn't poisoned
        claudeResponse.conversation_intent = priorIntent.split(',');
        claudeResponse.inquiry_quality = priorQuality;
        claudeResponse.business_value = session._lead.business_value || claudeResponse.business_value;
      }
    }
