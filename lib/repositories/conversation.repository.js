...
/**
 * Find the most recent active conversation for a contact regardless of agent binding.
 * @param {string} contactId - Contact UUID
 * @returns {Promise<Object|null>} - Conversation object or null
 */
export async function findLatestActiveConversation(contactId) {
 ...
 */
export async function createConversation(contactId, agentId = null, waPhoneNumberId = null, metaAdId = null) {
  const { data, error } = await supabase
    .from('conversations')
    .insert({
 ...
 */
export async function getOrCreateConversation(contactId, agentId = null, waPhoneNumberId = null) {
  const existing = await findActiveConversation(contactId, agentId);
  ...
  if (existing) {
    ...
    if (daysSinceLastMessage >= IDLE_THRESHOLD_DAYS) {
      ...
      return createConversation(contactId, agentId, waPhoneNumberId);
    }
