...
export async function replaceConversationLeads(conversationId, contactId, newLeads, { tenantId } = {}) {
  ...
  // Step 3: 批量插入新 leads。业务字段全部走 details JSONB；硬编码业务列已
  ...
  const leadsToInsert = newLeads.map(lead => {
    ...
    if (detailsObj.international_commercial_term !== undefined) {
      const normalized = normalizeIncoterm(detailsObj.international_commercial_term);
      if (normalized) detailsObj.international_commercial_term = normalized;
