 ...
 * 防止 partial-unique 索引冲撞，并级联清理它名下的 phones / ad_accounts ——
 * 否则那些行的 status 还是 'active'，meta_phone_numbers 的 WABA 跨租户 trigger
 * 会把它们当作占位，挡住其他租户认领（孤儿资源问题）。
 */
export async function createConnection({
  ...
  // 找到该租户当前的 active connection（如果有）—— 用来后续清理其名下资源
  const { data: oldActives } = await supabase
    .from('meta_connections')
    .select('id')
    .eq('tenant_id', tenantId)
    .eq('status', 'active');
  ...
  // 级联清理：和 disconnect 端点一样硬删 phones / ad_accounts，让 WABA / phone
  // / ad_account 立即释放，可被其他租户或本租户的新连接认领
  for (const old of oldActives || []) {
    await removePhonesByConnection(old.id);
    await removeAdAccountsByConnection(old.id);
  }
...
export async function recordHealthCheck(connectionId, { success, failedCount }) {
  ...
  const shouldRevoke = !success && failedCount >= 3;
  if (shouldRevoke) {
  ...
  // Revoke 时与 disconnect 端点对齐：硬删 phones / ad_accounts，释放资源给其他
  // 租户或本租户的新连接。否则这些行还挂着 status='active'，会通过 WABA
  // trigger / phone PK 长期阻塞重连。
  if (shouldRevoke) {
    await removePhonesByConnection(connectionId);
    await removeAdAccountsByConnection(connectionId);
  }
}
