...
export async function deactivateProductLine({ tenantId, id }) {
  if (!tenantId) throw new Error('deactivateProductLine: tenantId required');
  ...
  return updateProductLine({ tenantId, id, updates: { is_active: false } });
}
