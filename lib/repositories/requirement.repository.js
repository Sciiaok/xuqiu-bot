...
function envSettings() {
  return {
    ...
    bitable_wiki_node_token: config.requirementBot.bitableWikiNodeToken,
...
export async function markFeishuMessageProcessed({ tenantId, messageId }) {
  requireTenantId(tenantId, 'markFeishuMessageProcessed');
  if (!messageId) return true;
  return mutateState(state => {
    state.processedFeishuMessages = Array.isArray(state.processedFeishuMessages)
      ? state.processedFeishuMessages
      : [];
    const key = `${tenantId}:${messageId}`;
    if (state.processedFeishuMessages.includes(key)) return false;
    state.processedFeishuMessages.push(key);
    if (state.processedFeishuMessages.length > 1000) {
      state.processedFeishuMessages = state.processedFeishuMessages.slice(-1000);
    }
    return true;
  });
}
