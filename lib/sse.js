 ...
 */
export async function drainToRedis(generator, key, { sessionId } = {}) {
  ...
  try {
    for await (const event of generator) {
      // Check stop signal every 5 events (avoid Redis round-trip on every event)
      if (sessionId && ++eventCount % 5 === 0) {
        const stopped = await redis.exists(`stop:${sessionId}`);
        if (stopped) {
          console.log(`[drainToRedis] stop signal received for session=${sessionId}, terminating generator`);
          await redis.xadd(key, '*', 'event', 'done', 'data', JSON.stringify({ stopped: true }));
          generator.return?.();
          return;
        }
      }
