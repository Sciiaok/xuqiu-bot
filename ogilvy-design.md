...
> 代码位置：`src/agents/ogilvy/`、`skills/overseas-ad-planning/`、`app/(app)/ogilvy/`、`app/api/ogilvy/`
...
Skill bundle 在 `skills/overseas-ad-planning/`：

```
skills/overseas-ad-planning/
  SKILL.md                                    # 6 阶段 SOP 主文档
  references/
    data-sources.md                           # 阶段 2 市场分析的数据源清单
    strategy-template.md                      # 阶段 3 广告策略模板
    meta-creative-specs.md                    # 阶段 4 Meta 素材尺寸 / 文案上限
    meta-api-template.md                      # 阶段 5 plan_json 雏形 / Meta 字段
    compliance-blacklist.md                   # 各市场禁词 / 合规底线
    creative-prompt-patterns.md               # 阶段 4 创意 prompt 模板
```

**关键差异**：Medici 的 references **按需拉取**（`read_skill_reference` 工具），Ogilvy 的 references **全部内联**到 system prompt 末尾（[skill-host-patch.md §6](src/agents/ogilvy/skill-host-patch.md)）。原因：Ogilvy 单会话不会跑很多轮，但每轮都可能要用 references，按需拉的工具往返反而更贵；而且 references 总量 ~10K tokens 进静态 cache 段，写一次跨会话复用。
...
skill 定义 6 个子阶段（[skills/overseas-ad-planning/SKILL.md](skills/overseas-ad-planning/SKILL.md)），host-patch 加第 7 个 CTW 蒸馏阶段：
