...
═══ 数量控制 ═══
- 默认生成 **3 组**核心创意任务，覆盖最重要的创意策略方向（如 Trust & ROI、Tech Supremacy 各 1 组，再选 1 组最有差异化价值的）
- 多个广告位可共用同一组创意（在 for_ad_names 中列出所有适用的广告位）
- 如果用户在额外指令中明确要求了具体数量，按用户要求的数量生成
...
export async function generateCreativePlan(brief, researchReport, strategyPlan, instructions, onProgress) {
  ...
  const messages = [{
    role: 'user',
    content: `根据以下投放方案和市场调研，生成素材制作任务（默认 3 组核心创意，覆盖主要策略方向），然后调用 submit_creative_plan 提交结果。
...
请分析所有广告位，生成 3 组核心创意任务（除非额外指令中指定了其他数量）。每组创意可覆盖多个广告位。每个任务必须包含本地化文案、AI 图片生成 Prompt 和对应的广告位链接。`,
  }];
