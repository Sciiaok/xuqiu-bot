...
═══ 图片 Prompt 优化要求 ═══
- 使用英文编写，适配 Midjourney/DALL-E 等主流 AI 绘图工具
...
export async function generateCreativePlan(brief, researchReport, strategyPlan, instructions, onProgress) {
  ...
  // Collect references in parallel with any setup; gracefully handle failure
  onProgress?.({ step: 'collecting_references', detail: '搜集竞品参考素材' });
  let references = [];
  try {
    references = await collectReferences({ researchReport, brief });
  } catch (err) {
    console.warn('[creative-plan] collectReferences failed, continuing with empty references:', err.message);
  }
  onProgress?.({ step: 'generating_plan', detail: '生成素材制作方案' });
