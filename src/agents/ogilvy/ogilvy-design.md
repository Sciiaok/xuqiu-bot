...
>
> 文档版本：2026-05-06（PR 1–4 交付 + 性能/视觉迭代 + 2026-04 末 skill 驱动魔改 + 2026-05 长输出续写 / 素材图回插对话流 / 全链路 LLM 用量埋点）
...
- **实现**：[creative.service.js](../src/agents/ogilvy/creative.service.js) 按 3 模型降级链调用 OpenRouter
  ...
  产物落 `aigc-assets` Supabase bucket + `aigc_assets` 表，session 关联放 `metadata.autopilot_session_id`。
...
`/api/autopilot/conversations/[id]/launch`，后端生成器流式返回进度。
...
消息从 DB `autopilot_messages` 顺序重放。`role=tool` 行默认不渲染——**唯一例外**是 `generate_ad_creative` 的成功 `tool_result`：把图片以 headline 作图说插进对话流，让用户在等方案落地的同时直接看到生成出的素材。其它工具（draft_ad_plan / web_search / read_skill_reference 等）仍隐藏。流式中：
