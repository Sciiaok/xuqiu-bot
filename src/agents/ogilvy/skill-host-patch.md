...
skill 阶段五照常完整执行（市场分析 / 10 章策划案 / 素材清单 / Meta 双文档）。**阶段五结束、用户在对话里明确确认后**，把整个方案蒸馏成单个 CTW 投放计划，调 `draft_ad_plan` 提交。
...
skill 主文档里多处出现 `read_skill_reference({name: "..."})` 调用建议（阶段二取 `data-sources`、阶段三取 `strategy-template`、阶段四取 `meta-creative-specs`、阶段五取 `meta-api-template`）——这些建议**全部作废**。

**所有四个 reference 已直接附在本提示词末尾的「附录 · 参考资料」一节中**，可以直接当作本提示词的一部分引用，无需任何工具调用即可读到。

⚠️ `read_skill_reference` 工具已被移除。任何对它的调用都会返回 `Unknown tool` 错误。直接读末尾附录即可。

---

下方紧接着是 dynamic 段（WhatsApp 号码列表 + 已上传产品图列表），再下方是 reference 附录。
