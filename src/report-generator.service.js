...
async function fetchTopProducts(tenantId, fromISO, toISO) {
  const { data, error } = await supabase
    .from('leads')
    .select('details')
    .eq('tenant_id', tenantId)
...
async function computeKPI(tenantId, fromISO, toISO, prevFromISO, prevToISO) {
  // Current period
...
async function collectReportInput(tenantId, type, periodStart, periodEnd) {
  const { fromISO, toISO, fromDate, toDate } = periodToISO(periodStart, periodEnd);
  const { prevFromISO, prevToISO } = computePreviousPeriod(periodStart, periodEnd);

  // All reports get aggregated data
  const [dailyData, supplyChains, countries, topProducts, kpi] = await Promise.all([
    buildReportData(tenantId, 'daily_report', fromISO, toISO, fromDate, toDate),
    fetchSupplyChainBreakdown(tenantId, fromISO, toISO),
    fetchCountryDistribution(tenantId, fromISO, toISO),
    fetchTopProducts(tenantId, fromISO, toISO),
    computeKPI(tenantId, fromISO, toISO, prevFromISO, prevToISO),
  ]);
...
// 报表挂哪个产品线：单产品线时直接取那条；0 或多产品线视为不归属。
// LLM 调用的 productLine 标签只用作审计/计费 tag，不影响内容生成。
function resolveSingleProductLine(productLines) {
  if (!Array.isArray(productLines) || productLines.length !== 1) return null;
  return productLines[0] || null;
}
 ...
 */
export async function generateReport({ tenantId, type, periodStart, periodEnd, productLines = [] }) {
  if (!tenantId) throw new Error('generateReport: tenantId required');
  ...
  try {
    const { input, kpi } = await collectReportInput(tenantId, type, periodStart, periodEnd);
    // 单产品线报表才挂 product_line tag；多产品线 / 全租户报表归 NULL
    const productLine = resolveSingleProductLine(productLines);
...
export async function retryReport(reportId) {
  ...
  if (!report.tenant_id) throw new Error(`Report ${reportId} missing tenant_id`);
  ...
  try {
    const { input, kpi } = await collectReportInput(
      report.tenant_id, report.type, report.period_start, report.period_end
    );
    const productLine = resolveSingleProductLine(report.product_lines);
 ...
 */
export async function reportExists(tenantId, type, periodStart, periodEnd) {
  const { data, error } = await supabase
    .from('ai_reports')
    .select('id, status')
    .eq('tenant_id', tenantId)
