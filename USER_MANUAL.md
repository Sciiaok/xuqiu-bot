...
- 点 "释放" → AI 立刻接回
- 默认接管 TTL **1 小时**：到点后下一条客户消息自动放开 AI（`checkAndExpireTakeover` 即时检测）
...
4. 客户消息进了 message_queue 但 process_after 没到（聚合窗口 15–30s，故意做出"打字延迟"效果，不是 bug）
5. 之前判过 `FAQ_END`，`conversations.faq_ended_at` 被置位 → 进入静默期（客户继续发只入库不喂 AI）。等客户从新广告链接进来（带 CTWA referral）会自动解封；或者等 3 天 idle 起新会话；急用就 inbox 手动清 `faq_ended_at`
6. Meta webhook 签名校验失败（没配 `META_APP_SECRET` 或 Meta 后台改过 secret）→ 看 app log 里有没有 `webhook.signature.invalid` warn
...
| **Route** | CONTINUE（继续 AI 谈）/ FAQ_END（FAQ 答完结 + 进入静默期，新广告点击解封）/ HUMAN_NOW（转人工） |
| **Takeover** | 人工接管，AI 暂停自动回复，1h TTL 自动放开 |
