...
/**
 * GET /api/knowledge/assets?product_line_id=xxx
...
export async function GET(request) {
  try {
    ...
    const productLineId = new URL(request.url).searchParams.get('product_line_id');
    if (!productLineId) return NextResponse.json({ error: 'product_line_id is required' }, { status: 400 });
 ...
 * fields: file, product_line_id, description?
 *
...
export async function POST(request) {
  try {
    ...
    if (!file || !productLineId) {
      return NextResponse.json({ error: 'file and product_line_id are required' }, { status: 400 });
    }
    const line = await findProductLineInTenant({ tenantId: ctx.tenantId, productLineId });
    if (!line) {
      return NextResponse.json({ error: 'Product line not found' }, { status: 404 });
    ...
    const storagePath = `${productLineId}/${Date.now()}_${safeName}`;
...
export async function PATCH(request) {
  try {
    ...
    // findProductLineInTenant check below would already block cross-tenant
    // mutations under normal data, but if a row exists with mismatched
    // tenant_id / product_line_id (data drift via service-role inserts), the
    ...
    if (!(await findProductLineInTenant({ tenantId: ctx.tenantId, productLineId: row.product_line_id }))) {
...
export async function DELETE(request) {
  try {
    ...
    // the product-line tenant check below). Prevents data-drift edge cases
    // where a row's tenant_id and product_line_id might disagree.
    const { data: row, error: fetchErr } = await supabase
      .from('kb_assets')
      .select('storage_path, product_line_id')
      .eq('id', assetId)
      .eq('tenant_id', ctx.tenantId)
      .maybeSingle();
    if (fetchErr) throw fetchErr;
    ...
    // 验该 asset 所属产品线归属当前 tenant —— 否则 asset_id 一旦泄露就能跨 tenant 删数据。
    if (!(await findProductLineInTenant({ tenantId: ctx.tenantId, productLineId: row.product_line_id }))) {
      return NextResponse.json({ error: 'Asset not found' }, { status: 404 });
    }
