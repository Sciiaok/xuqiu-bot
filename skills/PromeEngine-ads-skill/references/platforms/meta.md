...
## 五、Meta CTW 锁定字段(★ 阶段 6 蒸馏约束,不是阶段 2-5 的预设)

> 本节字段是**阶段 6 输出 plan_json 时**必须使用的锁定值——LeadEngine V_1.0 host
> 只能 launch CTW,所以最终 plan_json 必须长这样。
>
> 但阶段 2-5 **不预设 CTW**:那里你要根据目标市场实际可行性推荐路径,可能是 Lead
> Form / Web Conv / Google UAC / TikTok 等。详见 SKILL.md §0.3、§4、§8.1。
>
> 进入阶段 6 时如果原推荐不是 CTW,必须先告诉用户蒸馏 lose 什么,等用户拍板再
> 落以下字段。
...
### 6.1 形式 × 市场可行性矩阵(给阶段 2 推荐用)

| 市场分类 | CTW 推荐度 | Lead Form 推荐度 | Web Conv 推荐度 | 首选形式 |
|---|---|---|---|---|
...
> 阶段 2 推荐时,**首先看本矩阵**决定首推形式,**不是无脑给 CTW**。详细背景见
> [`compliance.md §2.6`](../compliance.md)。

> ⚠️ V_1.0 host 只支持 CTW 落地,所以推非 CTW 路径时阶段 6 仍要蒸馏成 CTW
> ([SKILL.md §8.1 Case C](../../SKILL.md)),且要明示用户 lose 什么。
