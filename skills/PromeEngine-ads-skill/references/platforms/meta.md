...
## 六、其他 Meta 广告形式(V_1.0 不默认走,但保留规范)

V_1.0 默认走 CTW。其他形式作为未来扩展预留:

| 广告形式 | objective | optimization_goal | destination_type | 何时考虑 |
|---|---|---|---|---|
| **CTW(默认)** | WHATSAPP_CONVERSATIONS | CONVERSATIONS | WHATSAPP | V_1.0 默认 |
| CTM(Click-to-Messenger) | MESSAGES | CONVERSATIONS | MESSENGER | 客户用 Messenger 沟通 |
| Instant Form | OUTCOME_LEADS | LEAD_GENERATION | -- | 需结构化 lead form 时 |
| Website Conversions | OUTCOME_SALES | OFFSITE_CONVERSIONS | -- | 有落地页 + Pixel 时 |
| App Install | OUTCOME_APP_PROMOTION | APP_INSTALLS | -- | App 推广 |
| DPA | OUTCOME_SALES | OFFSITE_CONVERSIONS | -- | 商品目录电商 |
| Awareness | OUTCOME_AWARENESS | REACH | -- | 品牌曝光 |

未来扩展这些形式时,在本文件追加对应章节。
