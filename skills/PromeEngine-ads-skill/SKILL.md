...
阶段 1 业务理解    收集需求 + 自动识别业务模式
   ↓
阶段 2 路径选择    决定 D1×D2×D3 + 自动查规则
   ↓
阶段 3 市场分析    对选定平台/国家做调研
   ↓
阶段 4 投放策略    9 子节策划(预算/受众/层级/KPI)
   ↓
阶段 5 创意策略    4 步走(主线/矩阵/生图/绑首响)
   ↓
阶段 6 方案输出    plan_json + 后台手册(宿主接续 draft_ad_plan)
```
...
推荐逻辑(内部用,不展示):
- B2B 现货 → 优先 fb-feed + ig-feed + fb-non-feed(您 5 月数据 FB Feed 是金牌)
- 移动端为主 → mobile-android(80%)+ mobile-ios(20%)
- 价格敏感新兴市场 → 优先 fb-feed(CPM 低)
- 高 AOV 市场 → 加 ig-feed(用户决策更慎重,UI 信任感)

### 4.2 自动查规则(内部完成,不展示过程)

AI 用 `web_search` 自动查询:
- 目标平台广告规则(<3 个月)
- 目标国家广告法(<3 个月)
- 目标产品在目标国家的进口/合规限制(<6 个月)
- 各国文化禁忌(<12 个月)

结果存入内部 `platform_country_rules`,后续阶段引用。

### 4.3 广告形式锁定

V_1.0 锁定 **Meta CTW**(Click-to-WhatsApp)为唯一广告形式。其他形式(CTM / Instant Form / Website Conv / DPA / App Install / Awareness)虽在 references/platforms/meta.md 有规范,但 V_1.0 默认走 CTW。

**CTW 锁定字段**(plan_json 必须使用,不可改):
...
**资源位**:FB 信息流 + IG 信息流 + FB 非信息流(Marketplace/Reels)
**设备**:Mobile-Android(主)+ Mobile-iOS(扩量测试)

**理由**:
- 您 5 月数据 FB CTR 3.77% 远超 IG 1.29%,FB 是金牌渠道
- 目标 6 国均属价格敏感/枢纽市场,CTW 最适合
- Android 占 80%,iOS CPA 高 65% 但 CTR 高 31%,值得分组测试

✅ 已了解 Meta 平台和目标国家的投放规则

确认这个路径吗?
```

不展示:平台规则查询过程 / 字符限制锁定细节 / API 字段映射。
...
- `welcome_message` 字符长度按 §4.2 内部锁定的平台规则填,**不写死 300**
...
3. **CTW 锁定**:V_1.0 锁定 Meta CTW 单一形式,所有锁定字段不可改
...
11. **welcome_message 动态字符**:按 §4.2 锁定的平台规则,不写死 300
