/**
 * Strategy agent stability test — runs generateCampaignPlan N times
 * against real production brief+research data to measure success rate.
 *
 * Usage: node tests/stability/strategy-stability.test.js [session_id]
 */
import dotenv from 'dotenv';
dotenv.config({ path: '.env.local' });

const { default: supabase } = await import('../../lib/supabase.js');
const { generateCampaignPlan } = await import('../../src/strategy-agent.service.js');
...
async function loadSessionData(sessionId) {
  ...
  if (!research) {
    // Borrow research data from another session that has it
...
async function main() {
  console.log(`\n🔬 Strategy Stability Test — ${RUNS} runs against session ${SESSION_ID}\n`);
  ...
  for (let i = 0; i < RUNS; i++) {
    ...
    try {
      const plan = await generateCampaignPlan(brief, research, null, (step) => {
  ...
  console.log(`\n📊 Results: ${successes.length}/${RUNS} success (${(successes.length / RUNS * 100).toFixed(0)}%)`);
